...
Camera::Camera()
{
	cameraInit(640, 480, {0, 0.35, -1});
}

Camera::Camera(int width, int height, Vector3d position)
{
	cameraInit(width, height, position);
}

// Camera initialization
void Camera::cameraInit(int width, int height, Vector3d position)
{
...
std::vector<unsigned char> Camera::getBMPData()
{
	std::vector<unsigned char> image;

	for (size_t pixel_index = 0; pixel_index < this->screen.size(); pixel_index++)
	{
		image.push_back((unsigned char)(this->screen[pixel_index].color.b * 255));
		image.push_back((unsigned char)(this->screen[pixel_index].color.g * 255));
		image.push_back((unsigned char)(this->screen[pixel_index].color.r * 255));
	}
...
void Camera::saveBMP(std::string filename)
{
	...
	std::vector<unsigned char> image = this->getBMPData();
	...
	{
		fwrite(&image[0] + (width * (height - i - 1) * 3), 3, width, f);
